...
<br />
{if $gBitSystem->isFeatureActive( 'user_bookmarks' ) and $gBitUser->hasPermission( 'bit_p_create_bookmarks' )}
...
{if $prev_offset >= 0}
[<a href="{$smarty.const.TIDBITS_PKG_URL}menu.php?find={$find}&amp;offset={$prev_offset}&amp;sort_mode={$sort_mode}">{tr}prev{/tr}</a>]&nbsp;
{/if}
{tr}Page{/tr}: {$actual_page}/{$cant_pages}
{if $next_offset >= 0}
&nbsp;[<a href="{$smarty.const.TIDBITS_PKG_URL}menu.php?find={$find}&amp;offset={$next_offset}&amp;sort_mode={$sort_mode}">{tr}next{/tr}</a>]
{/if}
...
<a href="{$smarty.const.TIDBITS_PKG_URL}menu.php?find={$find}&amp;offset={$selector_offset}&amp;sort_mode={$sort_mode}">
