...
<div class="display usertasks">
	...
	<div class="body">
		<div class="navbar">
			<ul>
				<li><a href="{$smarty.const.TIDBITS_PKG_URL}tasks.php?tasks_use_dates=y">{tr}Use dates{/tr}</a></li>
				<li><a href="{$smarty.const.TIDBITS_PKG_URL}tasks.php?tasks_use_dates=n">{tr}All tasks{/tr}</a></li>
			</ul>
		</div>
			...
			<table class="table data">
				...
				{section name=user loop=$tasks}
					<tr {if $tasks[user].status eq 'c'}style="text-decoration:line-through;"{/if} class="{cycle values='odd,even'} prio{$tasks[user].priority}">
						...
						<td><a href="{$smarty.const.TIDBITS_PKG_URL}tasks.php?task_use_dates={$task_use_dates}&amp;task_id={$tasks[user].task_id}&amp;offset={$offset}&amp;sort_mode={$sort_mode}&amp;find={$find}#tasks">{$tasks[user].title|escape}</a></td>
				...
				<tr>
					<td colspan="4">
						...
						<input type="submit" class="btn btn-default" name="complete" value="{tr}Done{/tr}" />
						<input type="submit" class="btn btn-default" name="open" value="{tr}Not Done{/tr}" />
					</td>
					<td align="right">
						<input type="submit" class="btn btn-default" name="update" value="{tr}Update{/tr}" />
					</td>
				</tr>
			</table>
		{/form}
